...
const SPREADSHEET_ID = '1EsUEn9ZZ6STpFdSE2PsmvEkq6qVkSr7A3XH0MN5x8io'; // ← Remplacez uniquement ceci
...
function doPost(e) {
  try {
    ...
    switch (action) {
      ...
      case 'pushAllData':    result = pushAllData(body);                                                         break;
      default:                 result = { error: 'Action inconnue: ' + action };
    }
...
function initSpreadsheet() {
  ...
  if (missions.getLastRow() === 0) {
    missions.appendRow(['id', 'title', 'desc', 'cat', 'diff', 'xp', 'icon', 'freq', 'validation', 'due', 'secret', 'type', 'createdAt']);
    // Exemples
    missions.appendRow(['m_bed', 'Faire son lit', 'Ranger son lit dès le matin', 'Quotidien', 'facile', 15, '🛏️', 'quotidien', false, '', false, 'mission', new Date().toISOString()]);
    missions.appendRow(['m_read', 'Lire 20 minutes', 'Un livre de ton choix', 'Lecture', 'facile', 20, '📖', 'quotidien', false, '', false, 'mission', new Date().toISOString()]);
    missions.appendRow(['van_stars', 'Observer les étoiles', 'Trouver 3 constellations', 'Van', 'facile', 30, '🔭', 'unique', false, '', false, 'van', new Date().toISOString()]);
  }
...
const MISSION_HEADERS = ['id', 'title', 'desc', 'cat', 'diff', 'xp', 'icon', 'freq', 'validation', 'due', 'secret', 'type', 'createdAt'];
...
/* ══════════════════════════════════════════════════════════════
   PUSH COMPLET – Envoie toutes les données locales vers Sheets
   Appelée depuis l'app via action=pushAllData (POST)
══════════════════════════════════════════════════════════════ */

/**
 * Reçoit toutes les données locales de l'app et les écrit dans Sheets.
 * Utilisé pour la synchronisation initiale ou forcée.
 */
function pushAllData(data) {
  ...
  if (data.missions && data.missions.length) {
    const sheet = getOrCreateSheet(ss, SHEETS.MISSIONS);
    // Vide la feuille et réécrit tout
    if (sheet.getLastRow() > 1) sheet.deleteRows(2, sheet.getLastRow() - 1);
    const headers = ['id', 'title', 'desc', 'cat', 'diff', 'xp', 'icon', 'freq', 'validation', 'due', 'secret', 'type', 'createdAt'];
    if (sheet.getLastRow() === 0) sheet.appendRow(headers);
    data.missions.forEach(m => {
      sheet.appendRow(headers.map(h => m[h] ?? ''));
    });
    results.missions = data.missions.length;
  }

  // ── Récompenses ───────────────────────────────────────────
  if (data.rewards && data.rewards.length) {
    const sheet = getOrCreateSheet(ss, SHEETS.REWARDS);
    if (sheet.getLastRow() > 1) sheet.deleteRows(2, sheet.getLastRow() - 1);
    const headers = ['id', 'title', 'desc', 'cost', 'icon', 'available', 'createdAt'];
    if (sheet.getLastRow() === 0) sheet.appendRow(headers);
    data.rewards.forEach(r => {
      sheet.appendRow(headers.map(h => r[h] ?? ''));
    });
    results.rewards = data.rewards.length;
  }

  // ── Utilisateur ────────────────────────────────────────────
  if (data.user) {
    const sheet = getOrCreateSheet(ss, SHEETS.USERS);
    const headers = ['id', 'name', 'xp', 'totalXp', 'streak', 'lastActiveDate', 'level', 'missionsDone', 'avatarEmoji', 'createdAt'];
    if (sheet.getLastRow() === 0) {
      sheet.appendRow(headers);
      sheet.appendRow(headers.map(h => data.user[h] ?? ''));
    } else {
      ...
      for (let i = 1; i < sheetData.length; i++) {
        if (String(sheetData[i][idCol]) === String(data.user.id)) {
          headers.forEach((h, idx) => {
            if (data.user[h] !== undefined) sheet.getRange(i + 1, headerRow.indexOf(h) + 1).setValue(data.user[h]);
          });
          found = true; break;
        }
      }
      if (!found) sheet.appendRow(headers.map(h => data.user[h] ?? ''));
    }
    results.user = true;
  }
  ...
  if (data.history && data.history.length) {
    ...
    const headers = ['id', 'userId', 'type', 'title', 'icon', 'xp', 'createdAt'];
    if (sheet.getLastRow() === 0) sheet.appendRow(headers);
    // N'ajoute que les entrées absentes
    const existing = sheetToObjects(sheet).map(h => h.id);
    data.history.forEach(h => {
      if (!existing.includes(h.id)) {
        sheet.appendRow(headers.map(key => h[key] ?? ''));
      }
    });
